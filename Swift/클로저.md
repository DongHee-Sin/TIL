...
### escaping VS non-escaping 구분하는 이유
#### non-escaping클로저의 경우, 반드시 함수 내부에서만 사용되기 때문에 따로 힙영역에 클로저를 저장할 필요가 없고, 성능이 향상되기 때문
...
* 참조 타입 : 메모리 주소를 캡처 (weak/unowned 참조 선언 가능)
...
<br/>
 ...
 2. 새로운 스택영역(다른 스레드)을 생성하고, 생성된 스택영역에서 클로저를 실행시킨다. (스택프레임)
    * 새로운 스택영역에서 클로저를 참조한다. (RC+1) / 강한참조라면
